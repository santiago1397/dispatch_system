...
# tech arrived ("she's not there anymore", "no one home", "cx gone") is a
# CANCELLATION reason, not a plain decline — the job was accepted and
# worked, but couldn't be completed. Distinguished from ``REJECT_PHRASES``
# (which mean "we are not taking this job at all") so it can route to the
# ``canceled`` terminal status instead of ``rejected``. See
# :func:`is_cancel_signal`.
...
def is_repaste_with_note(body: str, job_body: str) -> bool:
    ...
    payment/settlement report (see :func:`_looks_like_payment_note`), or as
    the tech heading to the job (see :func:`_looks_like_en_route_note`)
    rather than an actual decline — an operator flagging "wrong number,
    pls check", reporting "Appt tomorrow 10:30 am", a tech closing out
    with "Total: 174$ cc", or "On way" back to the source chat is not
    passing on the job.
    """
