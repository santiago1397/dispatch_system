...
async def _extract_intent(db: AsyncSession, body: str) -> TechReplyIntent:
    ...
    prompt = (
        ...
        "'customer stopped answering, reached them later and they already got "
        "help elsewhere' (a tech report that the customer got help from "
        "someone else, got it resolved on their own, or no longer needs the "
        "tech — even after an earlier message said the tech was chasing an "
        "unresponsive customer — is canceled, reason=solved or no_service, "
        "not needs_follow_up).\n\n"
        "FIELDS:\n"
